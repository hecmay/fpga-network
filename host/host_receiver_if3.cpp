...
#define MY_IP_ADDR0 0xC0A80102 // 192.168.1.2
#define THEIR_IP_ADDR0 0xC0A80101 // 192.168.1
#define MY_IP_ADDR1 0xC0A80104 // 192.168.1.4
#define THEIR_IP_ADDR1 0xC0A80103 // 192.168.1.3
...
int main(int argc, char **argv) {
  ...
  unsigned int ip_gateway = argc >= 11
                                ? (unsigned int)strtol(argv[10], NULL, 16)
  ...
  unsigned int my_ip_address0 = argc >= 3
                                    ? (unsigned int)strtol(argv[2], NULL, 16)
                                    : (unsigned int)MY_IP_ADDR0;
  unsigned int their_ip_address0 = argc >= 5
  ...
  unsigned int my_ip_address1 = argc >= 4
                                    ? (unsigned int)strtol(argv[3], NULL, 16)
                                    : (unsigned int)MY_IP_ADDR1;
  unsigned int their_ip_address1 = argc >= 6
  ...
  // ------------------------------------
  // Only instantiate a loopback kernel (freerunning)
  return 0;
}
